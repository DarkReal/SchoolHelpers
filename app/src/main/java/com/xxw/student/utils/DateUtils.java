...
public class DateUtils {
	...
	public static String DateToString(Date date, int type) {
		...
		case 1:
			format = DATE_FORMAT2;
		case 2:
			format = TIME_FORMAT_SSS;
		case 3:
			format = DATE_NOFUll_FORMAT;
		case 4:
			format = TIME_NOFUll_FORMAT;
		default:
			format = DATE_FORMAT2;
			break;
		}
	...
	public static String stringFormat(String value) {
		...
		if (value.length() == 14) { // 长度为14格式转换成yyyy-mm-dd hh:mm:ss
			sReturn = value.substring(0, 4) + "-" + value.substring(4, 6) + "-"
		...
		if (value.length() == 19) { // 长度为19格式转换成yyyymmddhhmmss
			sReturn = value.substring(0, 4) + value.substring(5, 7)
	...
	public static String formatDate(String value) throws Exception {
		return getFormat(DateUtils.DATE_FORMAT2).format(
				string2Date(value, DateUtils.DATE_FORMAT2));
	}
	...
	public static String getCurrentWeek(Date value) throws Exception {
		...
		int weekIndex = calendar.get(Calendar.DAY_OF_WEEK) - 1 < 0 ? 0
