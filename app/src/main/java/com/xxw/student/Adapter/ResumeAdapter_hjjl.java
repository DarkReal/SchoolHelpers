 ...
 */
public class ResumeAdapter_hjjl extends BaseAdapter {

    //使用viewHolder提高效率
    public class TextViewHolder {
        //        教育背景
        TextView prizeName;
        TextView prize_degree;
        TextView prize_gettime;
    ...
    public ResumeAdapter_hjjl(Context context, Activity activity, List<HashMap<String, String>> list,
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {// 如果是第一次显示该页面(要记得保存到viewholder中供下次直接从缓存中调用)
            ...
            holder.prizeName = (TextView) convertView.findViewById(R.id.prizeName);
            holder.prize_degree = (TextView) convertView.findViewById(R.id.prize_degree);
            holder.prize_gettime = (TextView) convertView.findViewById(R.id.prize_gettime);
            holder.eachid = (TextView) convertView.findViewById(R.id.hjjl_list_id);
        ...
        holder.prizeName.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                et.setText(finalHolder.prizeName.getText().toString());

                //提交内容,更改对应的文本框
                submit.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (!finalHolder.prizeName.getText().toString().equals(et.getText().toString())) {
                            LogUtils.v("原值：" + finalHolder.prizeName.getText().toString() + "修改后的值" + et.getText().toString());
                            finalHolder.prizeName.setText(et.getText().toString());
                            currentItem = finalHolder.eachid.getText().toString();
                            LogUtils.v(currentItem + "rewardedName");
                            currentItemInt = getNumber(currentItem);
                            list.get(currentItemInt).put("rewardedName", et.getText().toString());
                            changed(-1);
                        }
                        alertDialog.cancel();
                    }
        ...
        holder.prize_degree.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                LayoutInflater inflater = LayoutInflater.from(mcontext);
                View viewinflator = inflater.inflate(R.layout.custom_alertdialog_edit, null);
                ...
                TextView submit = (TextView) viewinflator.findViewById(R.id.dialog_submit);
                TextView cancel = (TextView) viewinflator.findViewById(R.id.dialog_cancel);
                et.setText(finalHolder.prize_degree.getText().toString());

                //提交内容,更改对应的文本框
                submit.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (!finalHolder.prize_degree.getText().toString().equals(et.getText().toString())) {
                            LogUtils.v("原值：" + finalHolder.prize_degree.getText().toString() + "修改后的值" + et.getText().toString());
                            finalHolder.prize_degree.setText(et.getText().toString());
                            currentItem = finalHolder.eachid.getText().toString();
                            LogUtils.v(currentItem + "rewardedRank");
                            currentItemInt = getNumber(currentItem);
                            list.get(currentItemInt).put("rewardedRank", et.getText().toString());
                            changed(-1);
                        }
                        alertDialog.cancel();
                    }
        ...
        holder.prize_gettime.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                datePicker.setOnDatePickListener(new DatePicker.OnYearMonthPickListener() {
                    ...
                    public void onDatePicked(String year, String month) {
                        ...
                        if (!finalHolder.prize_gettime.getText().toString().equals(year + "-" + month)) {
                            LogUtils.v("原值：" + finalHolder.prize_gettime.getText().toString() + "修改后的值" + year + "-" + month);
                            finalHolder.prize_gettime.setText(year + "-" + month);
                            currentItem = finalHolder.eachid.getText().toString();
                            LogUtils.v(currentItem + "rewardedTime");
                            currentItemInt = getNumber(currentItem);
                            list.get(currentItemInt).put("rewardedTime", year + "-" + month);
                            changed(-1);
                        }
                    }
                });
    ...
    public HashMap<String, String> getHashMap() {
        ...
        hashMap.put("rewardedName", list.get(currentItemInt).get("rewardedName").toString());
        hashMap.put("rewardedRank",list.get(currentItemInt).get("rewardedRank").toString());
        hashMap.put("rewardedTime", list.get(currentItemInt).get("rewardedTime").toString());
        return hashMap;
    }
